function [ k_theta, theta_0 ] = force_angle_constant_special_case( atom_A, atom_B, atom_C, bond_lengths, eigenvalues, eigenvectors, coords, scaling_1, scaling_2 )
...
k_theta_array = zeros(180, 360);

for  theta = 1:180
    for phi = 1:360
        
        r = 1;
        u_N = [r * sind(theta)*cosd(theta),r * sind(theta)*sind(theta), r *cosd(theta) ];
        ...
        k_theta_array(theta,phi) = abs(k_theta_ij  * 0.5); %Change to OPLS form
        
    end
end

%Removes cases where u_N was linearly dependent of u_CB or u_AB
k_theta_array(isnan(k_theta_array)) = [];

%force constant used is taken as the mean
k_theta = mean(mean(k_theta_array)); 
k_theta
